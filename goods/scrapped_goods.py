...
class WildberriesScrappedGoods(Goods):
    ...
    def print(self) -> GoodsPrint:
        try:
            ...
            while page_number <= max_pages:
                web_page.get("https://www.wildberries.ru/__internal/u-search/exactmatch/ru/common/v18/search?ab_testing=false&appType=1&curr=rub&dest=-5818883&hide_vflags=4294967296&lang=ru&page=" + str(page_number) + "&query=" + str(query) +"&resultset=catalog&sort=popular&spp=30&suppressSpellcheck=false")
    ...
    def new(query: str) -> WildberriesScrappedGoods:
        return WildberriesScrappedGoods(
            GoodsOfhQuery(query, [])
        )
