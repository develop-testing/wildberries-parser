...
from product.fk_product import ProductData
...
class WildberriesCatalog:
    ...
    def scrab_of(self, query: str) -> None:
        goods = JsonFileCachedGoods.new(
            WildberriesScrappedGoods.new(query), "cache/goods.json"
        )
        ...
        for i in range(0, len(printout.products), pack_size):
            ...
            with ThreadPoolExecutor(max_workers=pack_size) as executor:
                ...
                for product_print in result:
                    if float(product_print.raiting) < 4.5 and int(product_print.price) < 10000:
